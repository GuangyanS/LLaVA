...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            self.ocr_tower = build_vision_tower(config, load_model = "ocr", delay_load=True)

            self.mm_projector = build_vision_projector(config)
            #HACK specify m = 2
            self.dino_mm_projector = build_vision_projector(config, load_model = "dino", m = 2)
            self.ocr_mm_projector = build_vision_projector(config, load_model = "ocr", m = 2)
            self.fusion_mm_projector = build_vision_projector(config, load_model = 'fusion')
    ...
    def get_vision_tower(self, load_model = "clip"):
        if load_model == "clip":
            ...
            vision_tower = getattr(self, 'ocr_tower', None)
            if type(vision_tower) is list:
                vision_tower = vision_tower[0]
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if self.get_vision_tower(load_model = "ocr") is None:
            ocr_tower = build_vision_tower(model_args, load_model="ocr")
        ...
        if getattr(self, 'dino_mm_projector', None) is None:
            self.dino_mm_projector = build_vision_projector(self.config, load_model = "dino", m = 2)
            
        if getattr(self, 'ocr_mm_projector', None) is None:
            self.ocr_mm_projector = build_vision_projector(self.config, load_model = "ocr", m = 2)

        if getattr(self, 'fusion_mm_projector', None) is None:
            self.fusion_mm_projector = build_vision_projector(self.config, load_model = 'fusion')
            
        if pretrain_mm_mlp_adapter is not None:
            mm_projector_weights = torch.load(pretrain_mm_mlp_adapter, map_location='cpu')
            def get_w(weights, keyword):
                return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}
            #TODO for load the correct mm_projector
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_ocr_vision_tower(self):
        return self.get_model().get_vision_tower(load_model = "ocr")
    ...
    def encode_images_withocr(self, images, m = 2):
        image_features_ocr = self.get_model().get_vision_tower(load_model = "ocr")(images)
    ...
    def prepare_inputs_labels_for_multimodal_dino_ocr(
        ...
        if images_crop is not None:
            if type(images_crop) is list or images_crop.ndim == 5:
                ...
                image_features_crop = self.encode_images_withexp(images_crop)
        else:
